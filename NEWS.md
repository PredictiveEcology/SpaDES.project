...
* `setupProject()`: `capture_dots()` no longer eagerly evaluates `...`; proxy bindings lazy-eval in the user's caller frame (captured before `withCallingHandlers`), fall back to the unevaluated expression on eval error / `NULL`, and are installed only for firstSet dots so `dotsLater` entries stay captured until their per-entry eval pass.
