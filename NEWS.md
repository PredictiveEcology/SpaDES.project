...
## Bug fixes
* `pathBuild()` positional calls take their field names from the bare symbols passed (e.g. `pathBuild(.ELFind, .samplingRange, .GCM, .SSP, .rep)`) when those name the same fields as the cached `scenarioFields()` in another order. A matching field *count* used to be enough for the cache to win, so a session that had read a queue whose columns were ordered differently built paths with the values named wrongly: with `withFieldLabel = c(.SSP = "ssp", .rep = "rep")` registered, `.rep`'s label landed on the sampling range and runs wrote to `outputs/<ELF>/rep1990-2020/<GCM>/ssp370/1`. Symbols that do *not* name cached fields are still treated as local variables, so the cache continues to name the fields.
...
* `setupProject()` / `getModule()` accept several `paths$modulePath` entries, finding each local module where it is; nested GitHub modules are flattened into the `modulePath` they are downloaded to (was "the condition has length > 1").
* `setupProject(useGit = )` works with several `paths$modulePath` entries (clones into the one holding the repo, else the first); a clone into a project repo with no commits no longer lands in `modulePath/<modulePath>/`.
* `getModule()` checks version specs on modules already on disk (e.g. `"Acct/modA@main (>= 0.5)"`); they were always reported "failed".
