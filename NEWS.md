...
* `plotChangeOverTime()` defaults to `terra::map.pal("differences")` (blue→white→red, purpose-built for difference maps) instead of `hcl.colors("RdBu", rev = TRUE)`. Falls back to `hcl.colors()` for any palette name `terra::map.pal()` doesn't recognise. Also now renders a continuous colour-bar legend (new `legendPosition` arg; default `"bottomright"`) keyed to the symmetric, zero-centred colour ramp the raster uses.
