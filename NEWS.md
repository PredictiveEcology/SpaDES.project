...
* Fixed `setupProject()` dropping **unnamed** repositories from `options(repos)` when ensuring a `CRAN` entry exists. The previous logic kept only *named* non-CRAN entries, so a repo added via `options(repos = unique(c(extraRepo, getOption("repos"))))` -- where `unique()` strips the names -- was silently removed. This broke `Require.noRemotes`: with the (typically unnamed) `r-universe` repo gone, the PE packages could not be resolved and installs fell back to slow per-ref source builds / failures. All existing repos (named or unnamed) are now preserved; only a `CRAN`-named entry is guaranteed.
