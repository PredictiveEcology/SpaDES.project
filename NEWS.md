...
* `setupProject()` / `getModule()` accept several `paths$modulePath` entries, finding each module (including nested GitHub modules, flattened in place) in whichever one holds it (was "the condition has length > 1").
