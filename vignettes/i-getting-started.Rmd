...
SuggestedPkgsNeeded <- c("SpaDES.core", "igraph", "visNetwork", "ellipsis")
...
```{r setCRAN,eval=TRUE,echo=FALSE}
...
```{r setCRAN_again,eval=TRUE,echo=FALSE}
## cleanup / restore state
if (is.null(getOption("repos")) || !"CRAN" %in% names(getOption("repos"))) {
 options(repos = c(CRAN = "https://cloud.r-project.org"))
}
...
```{r clean10,eval=TRUE,echo=FALSE}
## cleanup / restore state
# SpaDES.project::.teardownProject(out$paths, origLibPaths)
...
```{r clean11,eval=TRUE,echo=FALSE}
## cleanup / restore state
# SpaDES.project::.teardownProject(out$paths, origLibPaths)
...
```{r clean12,eval=TRUE,echo=FALSE}
## cleanup / restore state
# SpaDES.project::.teardownProject(out$paths, origLibPaths)
