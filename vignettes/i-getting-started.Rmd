...
The following will set an option as declared (i.e., `reproducible.useTerra = TRUE`), plus read the local `inst/options.R` file (with relative path), plus download and read the cloud-hosted `PredictiveEcology/SpaDES.project@development/inst/options.R` file.
...
.nodes <- 2
## TODO REMOVE-AFTER-DEBUG: capture state + traceback into stderr. We use
## withCallingHandlers (not tryCatch) so the call stack is intact at the
## moment of error — withCallingHandlers's handler runs in the throwing
## frame's context, so traceback() / sys.calls() see the real frames. We
## also write directly to stderr() (bypassing message() and avoiding any
## condition-handler entanglement) and explicitly avoid rlang::trace_back
## which appeared to re-trigger the error from inside the prior tryCatch.
.dbg <- function(...) writeLines(paste0(...), con = stderr())
out <- withCallingHandlers(
  SpaDES.project::setupProject(.mode = .mode, .nodes = .nodes),
  error = function(e) {
    .dbg("")
    .dbg("!!!! DEBUG: setupProject(.mode=.mode) crashed !!!!")
    .dbg("R:              ", R.version.string)
    .dbg("user:           ", Sys.info()[["user"]])
    .dbg("cwd:            ", getwd())
    for (p in .libPaths()) .dbg("libPath:        ", p)
    .dbg("SpaDES.project: ", as.character(packageVersion("SpaDES.project")))
    .dbg("Require:        ", as.character(packageVersion("Require")))
    .dbg("Error message:  ", conditionMessage(e))
    .dbg("Error class:    ", paste(class(e), collapse = ","))
    if (!is.null(conditionCall(e)))
      .dbg("Error call:     ", paste(deparse(conditionCall(e)), collapse = " "))
    .dbg("sys.calls() frames:")
    sc <- sys.calls()
    for (i in seq_along(sc)) {
      d <- paste(deparse(sc[[i]], width.cutoff = 200), collapse = " ")
      if (nchar(d) > 240) d <- paste0(substr(d, 1, 240), " ...")
      .dbg(sprintf("  [%2d] %s", i, d))
    }
    .dbg("!!!! END DEBUG !!!!")
    .dbg("")
  }
...
out <- setupProject(
  options = list(
    ...
    "PredictiveEcology/SpaDES.project@development/inst/options.R", # remote file -- sets many options
