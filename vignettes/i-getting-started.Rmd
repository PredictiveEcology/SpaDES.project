...
a <- setupOptions(options = list(
    "inst/options.R",                  # many items set because this is a file
    maxMemory = 6e+9,                  # if only 1 item, doesn't need to be in a list
    if(user("emcintir"))               # an "if" statement is OK
        list(map.overwrite = TRUE,     # two items, so in a list
             map.runAgain = T)
))
```  
...
Some paths are relative to the `projectPath`, except for 2 specific exceptions: `packagePath` is not placed within the `projectPath` and all "scratch" i.e., "temporary" folders. `packagePath` is placed in the `tools::R_user_dir(name)` folder, which is project specific, but does not clutter the `projectPath` with hundreds of folders that can interfere with many user actions especially in Rstudio. For example, "Find in Files" will take much longer (especially on Windows) if all the package files are in the `projectPath` (there are ways around this, but they require custom setups).
...
out <- setupProject(
  ...
  package = NULL # for this example, skip installation
)
```
