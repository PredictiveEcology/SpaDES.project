...
#' @param .envir The environment to look in for objects. This is used internally
#'   by the `setup*` functions and should be left unset by a user.
#' @param dots Any other named objects passed as a list a user might want for other elements.
#'   Combined with `...`
#' @param ... Any other named objects a user might want. Combined with `dots`.
#'
#' @export
#'
...
setupProject <- function(name, paths, modules, packages,
                         ...
                         .envir = environment(), dots, defaultDots, ...) {

  dotsToHere(dots, defaultDots, ...)
...
evalSUB <- function(val, valObjName, .envir, envir2) {
  ...
  if (is(val2, "list") && !is.null(names(val2))) {
    env <- environment() # This MUST be here because it is about updating the current object; not the one in the parent.frame()
...
dotsToHere <- function(dots, defaultDots, ..., .envir = parent.frame()) {
  if (missing(dots))
    dots <- list(...)
  else
    dots <- append(dots, list(...))
  if (!missing(defaultDots))
    dots <- Require:::modifyList2(defaultDots, dots)
