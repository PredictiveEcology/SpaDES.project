...
testRemoteCluster <- function(host, workers = 32L) {
  ...
  writeLines(c(
    ...
    'if (!is.null(cl)) {',
    '  message("[mcp_test] cluster up -- running worker jobs (generates PTY output)...")',
    '  # Each worker loads terra (GDAL messages) and writes to stderr (the SSH PTY).',
    '  # This reproduces what the SpaDES simulation does.',
    '  parallel::clusterEvalQ(cl, {',
    '    suppressPackageStartupMessages(library(terra))',
    '    r <- terra::rast(nrows=100, ncols=100)',
    '    terra::values(r) <- runif(terra::ncell(r))',
    '    message("worker pid=", Sys.getpid(), " done")',
    '    TRUE',
    '  })',
    '  message("[mcp_test] jobs done -- stopping cluster")',
    '  parallel::stopCluster(cl)',
    '  message("[mcp_test] SUCCESS")',
    '} else {',
